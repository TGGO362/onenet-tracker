  ...
  if (found) {
    ...
    if (match) {
      ...
      if (validCoord(lng, lat)) {
        const ts = parseTime(found.time);
        console.log(`[Parse] 定位(string): type=${type} lng=${lng} lat=${lat} ts=${ts}`);
        return { type, lng, lat, ts };
      }
    }
  }

  // ===== 格式 2: 设备自定义 JSON（PronetSetSendCh 发送） =====
  // GPS: { lngt: 117.38, latt: 31.94, isFix: 1 }
  // LBS: { lbs: { lbslng: 117.38, lbslat: 31.94 } }
  const custom = scanCustomCoords(body);
  if (custom) {
    const ts = custom.time ? parseTime(custom.time) : new Date().toISOString();
    console.log(`[Parse] 定位(custom): type=${custom.type} lng=${custom.lng} lat=${custom.lat} ts=${ts}`);
    return { type: custom.type, lng: custom.lng, lat: custom.lat, ts };
  }
...
function parseTime(timeVal) {
  ...
  if (num > 1000000000000) return new Date(num).toISOString();
  if (num > 1000000000) return new Date(num * 1000).toISOString();
  return new Date(timeVal).toISOString();
}

/** 扫描自定义 JSON 中的 GPS/LBS 坐标 */
function scanCustomCoords(obj, depth) {
  if (!obj || depth > 10) return null;
  if (Array.isArray(obj)) {
    for (const item of obj) {
      const r = scanCustomCoords(item, depth + 1);
      if (r) return r;
    }
    return null;
  }
  ...
  // GPS 坐标: lngt/latt（Air780EP libgnss 字段名）
  if ('lngt' in obj && 'latt' in obj) {
    const lng = parseFloat(obj.lngt);
    const lat = parseFloat(obj.latt);
    if (validCoord(lng, lat)) {
      return { type: 'gps', lng, lat, time: obj.at || obj.time || null };
    }
  }

  // LBS 坐标: lbs.lbslng/lbslat
  if (obj.lbs && typeof obj.lbs === 'object') {
    const lng = parseFloat(obj.lbs.lbslng);
    const lat = parseFloat(obj.lbs.lbslat);
    if (validCoord(lng, lat)) {
      return { type: 'base_station', lng, lat, time: obj.at || obj.time || null };
    }
  }
  ...
  for (const key of Object.keys(obj)) {
    if (typeof obj[key] === 'object' && obj[key] !== null) {
      const r = scanCustomCoords(obj[key], depth + 1);
      if (r) return r;
    }
  }
  return null;
